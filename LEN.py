...
# Select the number of features to use
num_features = 10  # Adjust the number of features as needed (10, 50, 100, 200, 500, 1000, 2000 and full features were used)

# Get the selected columns from the dictionary
selected_columns = features_dict[f"{num_features}_features"]

# Load the data with only the selected columns
data = pd.read_csv('malware_dataset.csv', encoding='utf8', usecols=selected_columns)

#data.head()
# Load the data with full features
#data = pd.read_csv('malware_dataset.csv', encoding='utf8')
